...
<Mermaid
  caption="Phoenix request flow: reads hit the database directly, while writes are ordered through Kafka before being persisted."
  code={`flowchart TB
      GATEWAY["Gateway"]

      GATEWAY -->|"read RPC"| READ["Read layer"]
      READ --> DB["Database"]

      GATEWAY -->|"write RPC"| WRITE["Write layer"]
      WRITE -->|"enqueue"| KAFKA["Kafka"]
      KAFKA -->|"consume ordered transactions"| WRITER["Writer"]
      WRITER --> DB
  `}
/>
...
<Mermaid code={`flowchart TB
    ...
    subgraph PG["Postgres database"]
      NODE["node: readable ID to UUID"]

      subgraph ACCOUNT["Account state"]
        BALANCE["balance"]
        NONCE["nonce"]
        STORAGE["storage"]
        CODE["code"]
      ...
      subgraph HISTORY["Chain history"]
        BLOCKS["blocks"]
        TX["transactions"]
        RECEIPTS["receipts"]
        EVENTS["event logs"]
      ...
      subgraph RUNTIME["Node metadata"]
        SNAPSHOT["snapshot"]
        FILTERS["websocket filters"]
        UNLOCKED["unlocked accounts"]
      end
    end
...
<Mermaid code={`flowchart LR
    ...
    ETH -->|Fork @ 100| N1[Node 1]

    N1 -->|Fork @ 110| N2[Node 2]

    N2 -. account miss .-> N1
    N1 -. account miss .-> CACHE[Mainnet Cache]
    CACHE -. cache miss .-> ETH
`} />
